...
export default function SecondSection() {
  ...
  return (
    // Menambahkan overflow-hidden agar aman jika ada elemen yang sedikit keluar batas di mobile
    <section className="bg-[#ffffff] flex flex-col items-center text-center px-4 w-full overflow-hidden">

      {/* 1. Teks Judul */}
      {/* Diperbarui: Ukuran font lebih kecil di mobile (32px) dan kembali ke 48px di layar medium (md) */}
      <h2 className="mt-10 md:mt-16 mb-8 md:mb-10 text-[32px] md:text-[48px] font-extrabold leading-tight">
      ...
      {/* Diperbarui: Jarak gap diperkecil di mobile (gap-4) dan membesar di layar medium (gap-8) */}
      <div className="mt-6 md:mt-10 mb-8 md:mb-10 w-full max-w-6xl grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8 justify-items-center">
        {characters.map((char, index) => (
          ...
          >
            {/* Diperbarui: Ukuran wrapper gambar mengecil di mobile dan membesar di layar tablet/desktop */}
            <div className="w-[140px] h-[140px] sm:w-[180px] sm:h-[180px] md:w-[220px] md:h-[220px] relative flex justify-center items-center">
              <Image
                ...
                width={220}
                height={220}
                // Diperbarui: w-full h-full agar gambar selalu mengikuti ukuran div parent-nya
                className="w-full h-full object-contain drop-shadow-sm group-hover:drop-shadow-lg transition-all duration-300"
              />
            </div>

            {/* Teks Deskripsi */}
            {/* Diperbarui: Ukuran font dan margin-top (mt) disesuaikan agar tidak kebesaran di mobile */}
            <p className={`mt-4 md:mt-10 text-[18px] sm:text-[22px] md:text-[28.5px] font-bold font-['8-Heavy'] transition-colors duration-300 whitespace-pre-line text-center ${char.colorClass}`}>
              {char.name}
            </p>
          </div>
        ))}
      ...
      {/* Diperbarui: Mengatur margin-bottom agar tidak terlalu jauh spasinya di HP */}
      <Link
        href="/belanja"
